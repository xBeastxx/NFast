...
function App() {
  ...
  const [transferDetails, setTransferDetails] = useState<{
    ...
    eta: string;
    fileName: string;
  }>({ stage: 'idle', percent: 0, speed: '', eta: '', fileName: '' });
  ...
  useEffect(() => {
    ...
    const removeProgress = window.ipcRenderer.croc.onProgress((_e, flow) => {
      ...
      // Regex to capture: Percentage, Speed, ETA
      const percentMatch = flow.match(/(\d+)%/);
      const speedMatch = flow.match(/, ([^)]+)\)/); // Matches ", 23 MB/s)" -> "23 MB/s"
      // ETA is tricky in standard croc, often just [2s:0s].
      ...
      setTransferDetails(prev => ({
        ...
        eta: '' // Croc output varies, keep simple for now
      }));
    });
    ...
    const removeFinished = window.ipcRenderer.croc.onFinished((_e, code) => {
      ...
      setTimeout(() => {
        ...
        setTransferDetails({ stage: 'idle', percent: 0, speed: '', eta: '', fileName: '' });
      }, resetDelay);
    });
              ...
              {(!hasCode || selectedContact) && !isLoading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 w-full max-w-lg animate-in fade-in zoom-in duration-500 p-2">
                  ...
                  <div className="relative group w-full">
                    <div className="text-3xl md:text-4xl font-mono text-cyan-400 font-bold tracking-wider select-text px-6 py-4 bg-black/40 rounded-xl border border-cyan-500/30 shadow-[0_0_30px_rgba(0,240,255,0.15)] break-all">
                      {generatedCode}
                    </div>
                    <button
            ...
            <h3 className="text-xs uppercase tracking-widest text-white/40 font-bold mb-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title={t('dashboard.force_reset_tooltip') || "Force Reset"}
                >
                  ↻
                </button>
                <button
                ...
                <div className="flex-1 flex flex-col justify-between animate-in fade-in duration-300">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className={`flex items-center gap-3 transition-opacity ${transferDetails.stage !== 'idle' ? 'opacity-100' : 'opacity-30'}`}>
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-white/90">{t('status.connecting')}</span>
                        <span className="text-[10px] text-white/40">{transferDetails.stage === 'connecting' ? 'Handshake...' : 'Secure Channel'}</span>
                      </div>
                    </div>
